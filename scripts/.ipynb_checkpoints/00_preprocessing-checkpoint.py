...
def fasta_path(genome: str, genome_dir: Path) -> Path:
    """
    FASTA files: path to each is user specified
    """
...
def main():
    ...
    parser.add_argument("--target_genome", required=True, help="Path to target genome fasta files")
    parser.add_argument("--spikein_genomes", nargs='+', required=True,
                        help="Path to spike-in genome fasta files (1 or more)")
    parser.add_argument("--spikein_genomes_symbols", required=False, help="A list of symbols to use for spike-in genome labeling. Example: [dm6, sac3]")

    args = parser.parse_args()

    target_genome = args.target_genome
    spikein_genomes = args.spikein_genomes
    spikein_genomes_symbols = args.spikein_genomes_symbols
